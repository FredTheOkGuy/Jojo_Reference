...
export default function DetailScreen({ group, onBack, onChat, onLeave }: DetailScreenProps) {
        ...
        <Card className="rounded-[18px] p-6 mb-4">
          <div className="flex gap-4 mb-4">
            ...
            <div className="min-w-0">
              ...
              <h1 className="font-black text-3xl text-[#1a1610] font-['Syne'] leading-tight">{group.name}</h1>
            </div>
          </div>

          <p className="text-sm text-[#4a4438] leading-relaxed font-medium mb-5">{group.desc}</p>
          <div className="flex gap-2.5">
            <Button label="Open Chat" onClick={onChat} variant="primary" fullWidth />
            <Button label="Leave Group" onClick={onLeave} variant="danger" fullWidth />
          </div>
        </Card>

        <div className="grid grid-cols-2 gap-3.5 mb-4">
          <InfoBox label="📍 Location" value={group.location} />
          <InfoBox label="🕐 Schedule" value={group.days} subValue={group.time} />
          <InfoBox label="👥 Capacity" className="col-span-2" value={<CapacityMeter current={group.cur} max={group.max} />} />
        </div>
