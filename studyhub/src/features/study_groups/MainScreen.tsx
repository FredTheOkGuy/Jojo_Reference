...
export default function MainScreen({
  ...
  const courseCodes = [...new Set(openGroups.map((g) => g.filterCode).filter(Boolean))];
  const courseNumbers = [...new Set(openGroups.map((g) => g.filterNum).filter(Boolean))];
    ...
    <div className="flex flex-col min-h-screen bg-[#f2ede3]">
      ...
      <div className="h-16 bg-[#faf8f4] border-b border-[#ddd8cc] flex items-center px-5 sticky top-0 z-50">
        ...
        <div className="flex-1 text-center">
          <span className="font-black text-2xl text-[#c96332] font-['Syne'] -tracking-0.5px">StudyHub</span>
        </div>
      ...
      <div className="flex-1 px-5 py-6 max-w-2xl mx-auto w-full">
        ...
        <div className="flex items-center justify-between mb-3.5">
          <span className="text-xs font-bold uppercase tracking-wider text-[#9a9282]">Your Study Groups</span>
        ...
        {/* Open Study Groups */}
        <div className="flex items-center justify-between mb-3.5">
          <span className="text-xs font-bold uppercase tracking-wider text-[#9a9282]">Open Study Groups</span>
        </div>
          ...
          {filteredOpenGroups.length > 0 ? (
            filteredOpenGroups.map((group) => (
              <GroupCard
                ...
                onDetail={() => {
                  /* Not typically used for open groups, but available */
                }}
              />
            ))
          ) : (
