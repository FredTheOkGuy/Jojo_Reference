...
export default function ChatsScreen({ groups, onChat, onBack, onProfile }: ChatsScreenProps) {
  const myGroups = groups.filter((g) => g.joined);

  return (
    <div className="flex flex-col min-h-screen bg-[#f2ede3]">
      <TopBar title="Messages" left={<BackButton onClick={onBack} label="" />} right={<AvatarButton onClick={onProfile} />} />

      <main className="flex-1 px-5 py-5 max-w-2xl mx-auto w-full">
        {myGroups.length > 0 ? (
          myGroups.map((group) => <ChatListItem key={group.id} group={group} onOpen={onChat} />)
        ) : (
